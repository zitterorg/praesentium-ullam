# @zitterorg/praesentium-ullam

![CI](https://github.com/zitterorg/praesentium-ullam/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@zitterorg/praesentium-ullam.svg?style=flat)](https://www.npmjs.com/package/@zitterorg/praesentium-ullam)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)


`@zitterorg/praesentium-ullam` enables the use of [CORS](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing) in a Fastify application.

## Install
```
npm i @zitterorg/praesentium-ullam
```
...
## Usage
Require `@zitterorg/praesentium-ullam` and register it as any other plugin, it will add an `onRequest` hook and a [wildcard options route](https://github.com/fastify/fastify/issues/326#issuecomment-411360862).
```js
import Fastify from 'fastify'
import cors from '@zitterorg/praesentium-ullam'
...
fastify.register(require('@zitterorg/praesentium-ullam'), (instance) => {
  return (req, callback) => {
    const corsOptions = {
...
By default, `@zitterorg/praesentium-ullam` adds a `onRequest` hook where the validation and header injection are executed. This can be customized by passing `hook` in the options. Valid values are `onRequest`, `preParsing`, `preValidation`, `preHandler`, `preSerialization`, and `onSend`.
...
fastify.register(require('@zitterorg/praesentium-ullam'), {
  hook: 'preHandler',
